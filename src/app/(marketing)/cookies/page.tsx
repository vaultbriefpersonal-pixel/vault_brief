  ...
  return (
    <div style={{ paddingTop: 72 }}>
      <section className="vb-pad-x" style={{ paddingTop: 80, paddingBottom: 120, maxWidth: 720, margin: "0 auto" }}>
        ...
        <div className="vb-table-scroll" style={{ background: "#161616", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, overflow: "hidden", marginBottom: 32 }}>
          ...
          {COOKIES.map((c, i) => (
            <div key={c.name} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 2fr 1fr", padding: "14px 20px", borderBottom: i < COOKIES.length - 1 ? "1px solid rgba(255,255,255,0.06)" : "none", gap: 8 }}>
              <span style={{ fontFamily: "var(--font-geist-mono), monospace", fontSize: 13, color: "#00e87b" }}>{c.name}</span>
              <span style={{ fontFamily: "var(--font-inter), Inter, sans-serif", fontSize: 13, color: c.type === "Essential" ? "#f0f0f0" : "#888888" }}>{c.type}</span>
              <span style={{ fontFamily: "var(--font-inter), Inter, sans-serif", fontSize: 13, color: "#888888", lineHeight: 1.5 }}>{c.purpose}</span>
        ...
        <p style={s.p}>Questions about our cookie use? Email <a href="mailto:hello@vaultbrief.com" style={{ color: "#00e87b", textDecoration: "none" }}>hello@vaultbrief.com</a>.</p>
      </section>
    </div>
  );
}
